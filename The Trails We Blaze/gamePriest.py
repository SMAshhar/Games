# # # I may have named the mosters as alian_0, alian_1 etc, but it is not going to go the same way. # # # 
# # # The adventure is actually a medival, mythical one. # # #
...
with open(".\Priest\priest.csv") as f:           # Adding up the powers' details. 
    reader = csv.reader(f)                       # Taking out the power details from the CSV file
    for a in reader:
        powers += reader
...
def smite(enemyHp):                     # Defining each ability and its function
    enemyHp -= (15 + level*30)
    print(enemyHp)
    return enemyHp
...
while x == True:                # Starting the first loop to start the game
    ...
    while alien_0.HP > 0 and EliaHP > 0:        # The loop to start the fight 
        ...
        print("Elia Hp = ", EliaHP)         # Stating condition after each turn, 
